...
if (searchBar) {
  searchBar.addEventListener('input', (e) => {
    ...
    incidentCards.forEach(card => {
      ...
      const riskDisplay = risk === 'high' ? 'High threat' : 
                         risk === 'risk' ? 'Moderate threat' : 
                         risk === 'normal' ? 'Low threat' : risk;
...
function createIncidentCard(request) {
  ...
  const riskText = request.risk === 'HIGH' ? 'High threat' : request.risk === 'RISK' ? 'Moderate threat' : 'Low threat';
  ...
  riskButtons.forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (currentRiskDisplay) {
        currentRiskDisplay.textContent = selectedRisk === 'HIGH' ? 'High threat' : selectedRisk === 'RISK' ? 'Moderate threat' : 'Low threat';
